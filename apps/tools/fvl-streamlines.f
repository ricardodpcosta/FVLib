...
!|_ _ _ _ _ _ _ _ _ _ _ _ _|_ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _|
! About: track particles as streamlines
! Modification: March, 2025

#include "macros.f"

program fvl_streamlines_main
...
if(fvl_findopt("-h") .or. fvl_findopt("--help")) then
      call fvl_loginfo("About: Utility tool for computing streamlines.")
      call fvl_loginfo("Usage: fvl-streamlines <input_mesh_file> <input_data_file> <output_mesh_file> <output_data_file> [options]")
...
! compute streamlines
call fvl_loginfo("Computing streamlines from "//fvl_trim(datainpath)//" to "//fvl_trim(dataoutpath)//"...")
      ...
      if(fvl_trim(meshoutformat)=="fvm2") then
            if(fvl_trim(datainformat)=="fvd2") then
                  if(fvl_trim(dataoutformat)=="fvd2") then
                        call fvl_streamlines2d(meshinfile,datainfile,meshoutfile,dataoutfile,dictfile)
                  else
                        call fvl_logerror("Invalid output data format")
                  end if
            else
      ...
      if(fvl_trim(meshoutformat)=="fvm3") then
            if(fvl_trim(datainformat)=="fvd3") then
                  if(fvl_trim(dataoutformat)=="fvd3") then
                        call fvl_streamlines3d(meshinfile,datainfile,meshoutfile,dataoutfile,dictfile)
                  else
                        call fvl_logerror("Invalid output data format")
                  end if
            else
...
end program fvl_streamlines_main
! end of file
