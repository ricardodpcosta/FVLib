...
integer(kind=__fvl_integer_kind__)::i,j,nummeshes,convergence,p_fixconstant,tablesprecision,unit,element,&
      ...
      primal_numverts(maxnummeshes),diamond_numverts(maxnummeshes),steps(maxnummeshes),&
...
if(fvl_findopt("-h") .or. fvl_findopt("--help")) then
      ...
      call fvl_loginfo("Usage: fvl-postvert-thermoincompflow2d [options]")
...
! read dictionary
controldictfile            = fvl_dict_file(path=postdictpath,form="ascii",action="read")
...
do i=1,maxnummeshes
      ...
      if(exist1) then
            ...
            diamondmesh = primalmesh

            ! write mesh report
            call diamondmesh % report("- diamond mesh")

            ! initialize variables
            primal_numverts(i) = primalmesh%getnumverts()
            diamond_numverts(i) = diamondmesh%getnumverts()
...
#include "models.f"
      ...
      if(associated(t_valuefun)) then
            ...
            ! allocate memory
            call fvl_allocate(t_exactvals,primal_numverts(i))

            ! compute exact values
            call primalmesh % evalvertcentrevalues(t_valuefun,t_exactvals)
            ...
            call solutionsfile % writedata(t_exactvals,fvl_trim(t_label)//" Exact",fvl_data_file_vertcentrevalues,times(i),steps(i))
            ...
            call solutionsfile % open()
            element = 0
            do while(element/=fvl_data_file_vertcentrevalues)
            ...
            call t_reconstmap % mapvertcentrevalues(primalmesh,t_exactvals)
      ...
      if(associated(p_valuefun)) then
            ...
            ! allocate memory
            call fvl_allocate(p_exactvals,primal_numverts(i))

            ! compute exact values
            call primalmesh % evalvertcentrevalues(p_valuefun,p_exactvals)
            ...
            call solutionsfile % open()
            call solutionsfile % writedata(p_exactvals,fvl_trim(p_label)//" Exact",fvl_data_file_vertcentrevalues,times(i),steps(i))
            call solutionsfile % close()
            ...
            ! map reference solution
            call p_reconstmap % mapvertcentrevalues(primalmesh,p_exactvals)
      ...
      if(associated(u_valuefun)) then
            ...
            ! allocate memory
            call fvl_allocate(u_exactvals,2,primal_numverts(i))

            ! compute exact values
            call primalmesh % evalvertcentrevalues(u_valuefun,u_exactvals)
            ...
            call solutionsfile % open()
            call solutionsfile % writedata(u_exactvals,fvl_trim(u_label)//" Exact",fvl_data_file_vertcentrevalues,times(i),steps(i))
            call solutionsfile % close()
            ...
            ! allocate memory
            call fvl_allocate(u_refvals2,2*primalrefmesh%getnumverts())

            ! reshape data
            u_refvals2 = reshape(transpose(u_refvals1),[2*primalrefmesh%getnumverts()],order=[1])
            ...
            ux_reconstmap = fvl_reconstmap2d(primal_patch,primalrefmesh,primalmeshlocator,controldictfile,"reconst_map",&
                  u_refvals2(1:primalrefmesh%getnumverts()))
            uy_reconstmap = fvl_reconstmap2d(primal_patch,primalrefmesh,primalmeshlocator,controldictfile,"reconst_map",&
                  u_refvals2(primalrefmesh%getnumverts()+1:2*primalrefmesh%getnumverts()))
            ...
            ! allocate memory
            call fvl_allocate(u_exactvals,2,primal_numverts(i))
            call fvl_allocate(auxv,2*primal_numverts(i))

            ! map reference solution
            call ux_reconstmap % mapvertcentrevalues(primalmesh,auxv(1:primal_numverts(i)))
            call uy_reconstmap % mapvertcentrevalues(primalmesh,auxv(primal_numverts(i)+1:2*primal_numverts(i)))
            u_exactvals = reshape(auxv,[2,primal_numverts(i)],order=[2,1])
      ...
      if(associated(u_valuefun)) then
            ...
            ! allocate memory
            call fvl_allocate(diamond_u_exactvals,2,diamond_numverts(i))

            ! compute exact values
            call diamondmesh % evalvertcentrevalues(u_valuefun,diamond_u_exactvals)
            ...
            solutionsfile = fvl_data_file(path=fvl_trim(u_suffix),form=errorsfilesform(i),action="write")
            call solutionsfile % open()
            call solutionsfile % writedata(diamond_u_exactvals,fvl_trim(u_label)//" Exact",fvl_data_file_vertcentrevalues,times(i),steps(i))
            call solutionsfile % close()
            ...
            ! allocate memory
            call fvl_allocate(diamond_u_refvals2,2*diamondrefmesh%getnumverts())

            ! reshape data
            diamond_u_refvals2 = reshape(transpose(diamond_u_refvals1),[2*diamondrefmesh%getnumverts()],order=[1])
            ...
            diamond_ux_reconstmap = fvl_reconstmap2d(diamond_patch,diamondrefmesh,diamondmeshlocator,controldictfile,"reconst_map",&
                  diamond_u_refvals2(1:diamondrefmesh%getnumverts()))
            diamond_uy_reconstmap = fvl_reconstmap2d(diamond_patch,diamondrefmesh,diamondmeshlocator,controldictfile,"reconst_map",&
                  diamond_u_refvals2(diamondrefmesh%getnumverts()+1:2*diamondrefmesh%getnumverts()))
            ...
            ! allocate memory
            call fvl_allocate(diamond_u_exactvals,2,diamond_numverts(i))
            call fvl_allocate(auxv,2*diamond_numverts(i))

            ! map reference solution
            call diamond_ux_reconstmap % mapvertcentrevalues(diamondmesh,auxv(1:diamond_numverts(i)))
            call diamond_uy_reconstmap % mapvertcentrevalues(diamondmesh,auxv(diamond_numverts(i)+1:2*diamond_numverts(i)))
            diamond_u_exactvals = reshape(auxv,[2,diamond_numverts(i)],order=[2,1])
      ...
      if(exist1) then
            ...
            ! allocate memory
            call fvl_allocate(t_errorvals,primal_numverts(i))
            ...
            t_error2(i) = 0.0d0
            t_errorinf(i) = 0.0d0
            do j=1,primal_numverts(i)
                  t_error1(i) = t_error1(i)+abs(t_errorvals(j))
                  t_error2(i) = t_error2(i)+(t_errorvals(j)**2.0)
                  t_errorinf(i) = max(t_errorinf(i),abs(t_errorvals(j)))
            end do
            t_error1(i) = t_error1(i)/fvl_real(primal_numverts(i))
            t_error2(i) = sqrt(t_error2(i)/fvl_real(primal_numverts(i)))

            ! compute convergence orders
            if(i>1) then
                  if(convergence==1) then
                        if(primal_numverts(i-1)>0 .and. primal_numverts(i-1)/=primal_numverts(i) .and. t_errorinf(i)>0.0d0) then
                              t_order1(i) = 2.0d0*log(t_error1(i-1)/t_error1(i))&
                                    /log(fvl_real(primal_numverts(i))/fvl_real(primal_numverts(i-1)))
            ...
            solutionsfile = fvl_data_file(path=fvl_trim(t_suffix),form=errorsfilesform(i),action="write")
            call solutionsfile % open()
            call solutionsfile % writedata(t_errorvals,t_map,fvl_trim(t_label)//" Error",fvl_data_file_vertcentrevalues,times(i),steps(i))
            call solutionsfile % close()
      ...
      if(exist1) then
            ...
            ! allocate memory
            call fvl_allocate(p_errorvals,primal_numverts(i))
            call fvl_omp_clean(p_errorvals)

            ! fix constant
            if(p_fixconstant==1) then
                  call fvl_omp_sub(p_exactvals,fvl_omp_mean(p_exactvals))
                  call fvl_omp_sub(p_approxvals,fvl_omp_mean(p_approxvals))
            end if
            ...
            p_errorinf(i) = 0.0d0
            do j=1,primal_numverts(i)
                  p_error1(i) = p_error1(i)+abs(p_errorvals(j))
                  p_error2(i) = p_error2(i)+(p_errorvals(j)**2.0)
                  p_errorinf(i) = max(p_errorinf(i),abs(p_errorvals(j)))
            end do
            p_error1(i) = p_error1(i)/fvl_real(primal_numverts(i))
            p_error2(i) = sqrt(p_error2(i)/fvl_real(primal_numverts(i)))

            ! compute convergence orders
            if(i>1) then
                  if(convergence==1) then
                        if(primal_numverts(i-1)>0 .and. primal_numverts(i-1)/=primal_numverts(i) .and. p_errorinf(i)>0.0d0) then
            ...
            solutionsfile = fvl_data_file(path=fvl_trim(p_suffix),form=errorsfilesform(i),action="write")
            call solutionsfile % open()
            call solutionsfile % writedata(p_errorvals,p_map,fvl_trim(p_label)//" Error",fvl_data_file_vertcentrevalues,times(i),steps(i))
            call solutionsfile % close()
      ...
      if(exist1) then
            ...
            ! allocate memory
            call fvl_allocate(u_errorvals,2,primal_numverts(i))
            ...
            u_errorinf(i) = 0.0d0
            do j=1,primal_numverts(i)
                  ux_error1(i) = ux_error1(i)+abs(u_errorvals(1,j))
                  ux_error2(i) = ux_error2(i)+(u_errorvals(1,j)**2.0)
                  ux_errorinf(i) = max(ux_errorinf(i),abs(u_errorvals(1,j)))
                  uy_error1(i) = uy_error1(i)+abs(u_errorvals(2,j))
                  uy_error2(i) = uy_error2(i)+(u_errorvals(2,j)**2.0)
            ...
            ux_error1(i) = ux_error1(i)/fvl_real(primal_numverts(i))
            ux_error2(i) = sqrt(ux_error2(i)/fvl_real(primal_numverts(i)))
            uy_error1(i) = uy_error1(i)/fvl_real(primal_numverts(i))
            uy_error2(i) = sqrt(uy_error2(i)/fvl_real(primal_numverts(i)))
            u_error1(i) = u_error1(i)/fvl_real(primal_numverts(i))
            u_error2(i) = sqrt(u_error2(i)/fvl_real(primal_numverts(i)))

            ! compute convergence orders
            if(i>1) then
                  if(convergence==1) then
                        if(primal_numverts(i-1)>0 .and. primal_numverts(i-1)/=primal_numverts(i)) then
            ...
            solutionsfile = fvl_data_file(path=fvl_trim(u_suffix),form=errorsfilesform(i),action="write")
            call solutionsfile % open()
            call solutionsfile % writedata(u_errorvals,u_map,fvl_trim(u_label)//" Error",fvl_data_file_vertcentrevalues,times(i),steps(i))
            call solutionsfile % close()
      ...
      if(exist1) then
            ...
            ! allocate memory
            call fvl_allocate(diamond_u_errorvals,2,diamond_numverts(i))
            ...
            do j=1,diamond_numverts(i)
                  diamond_ux_error1(i) = diamond_ux_error1(i)+abs(diamond_u_errorvals(1,j))
                  diamond_ux_error2(i) = diamond_ux_error2(i)+(diamond_u_errorvals(1,j)**2.0)
                  diamond_ux_errorinf(i) = max(diamond_ux_errorinf(i),abs(diamond_u_errorvals(1,j)))
                  diamond_uy_error1(i) = diamond_uy_error1(i)+abs(diamond_u_errorvals(2,j))
                  diamond_uy_error2(i) = diamond_uy_error2(i)+(diamond_u_errorvals(2,j)**2.0)
            ...
            diamond_ux_error1(i) = diamond_ux_error1(i)/fvl_real(diamond_numverts(i))
            diamond_ux_error2(i) = sqrt(diamond_ux_error2(i)/fvl_real(diamond_numverts(i)))
            diamond_uy_error1(i) = diamond_uy_error1(i)/fvl_real(diamond_numverts(i))
            diamond_uy_error2(i) = sqrt(diamond_uy_error2(i)/fvl_real(diamond_numverts(i)))
            diamond_u_error1(i) = diamond_u_error1(i)/fvl_real(diamond_numverts(i))
            diamond_u_error2(i) = sqrt(diamond_u_error2(i)/fvl_real(diamond_numverts(i)))

            ! compute convergence orders
            if(i>1) then
                  if(convergence==1) then
                        if(diamond_numverts(i-1)>0 .and. diamond_numverts(i-1)/=diamond_numverts(i)) then
                              if(diamond_ux_errorinf(i)>0.0d0) then
                                    diamond_ux_order1(i) = 2.0d0*log(diamond_ux_error1(i-1)/diamond_ux_error1(i))&
                                          /log(fvl_real(diamond_numverts(i))/fvl_real(diamond_numverts(i-1)))
            ...
            solutionsfile = fvl_data_file(path=fvl_trim(u_suffix),form=errorsfilesform(i),action="write")
            call solutionsfile % open()
            call solutionsfile % writedata(diamond_u_errorvals,diamond_u_map,fvl_trim(u_label)//" Error",fvl_data_file_vertcentrevalues,times(i),steps(i))
            call solutionsfile % close()
...
! dofs
if(convergence==1) then
      primal_numdofs(1:nummeshes) = primal_numverts(1:nummeshes)
      diamond_numdofs(1:nummeshes) = diamond_numverts(1:nummeshes)
