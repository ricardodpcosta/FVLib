...
integer(kind=__fvl_integer_kind__)::i,j,nummeshes,convergence,p_fixconstant,tablesprecision,unit,element,&
      ...
      primal_numedges(maxnummeshes),diamond_numedges(maxnummeshes),steps(maxnummeshes),&
      linearsystemnumiters(maxnummeshes),fixpointnumiters(maxnummeshes)
real(kind=__fvl_real_kind__)::time,aux,edgelength,primalmeshlengths(maxnummeshes),diamondmeshlengths(maxnummeshes),&
...
if(fvl_findopt("-h") .or. fvl_findopt("--help")) then
      ...
      call fvl_loginfo("Usage: fvl-postedge-thermoincompflow2d [options]")
...
do i=1,maxnummeshes
      ...
      if(exist1) then
            ...
            primal_numedges(i) = primalmesh%getnumedges()
            diamond_numedges(i) = diamondmesh%getnumedges()
            primalmeshlengths(i) = fvl_omp_sum(primalmesh%getedgelengths())
            diamondmeshlengths(i) = fvl_omp_sum(diamondmesh%getedgelengths())
...
#include "models.f"
      ...
      if(associated(t_valuefun)) then
            ...
            ! allocate memory
            call fvl_allocate(t_exactvals,primal_numedges(i))

            ! compute exact values
            call primalmesh % evaledgemeanvalues(t_valuefun,t_exactvals)
            ...
            call solutionsfile % writedata(t_exactvals,fvl_trim(t_label)//" Exact",fvl_data_file_edgemeanvalues,times(i),steps(i))
            ...
            call solutionsfile % open()
            element = 0
            do while(element/=fvl_data_file_edgemeanvalues)
      ...
      if(associated(p_valuefun)) then
            ...
            ! allocate memory
            call fvl_allocate(p_exactvals,primal_numedges(i))

            ! compute exact values
            call primalmesh % evaledgemeanvalues(p_valuefun,p_exactvals)
            ...
            call solutionsfile % open()
            call solutionsfile % writedata(p_exactvals,fvl_trim(p_label)//" Exact",fvl_data_file_edgemeanvalues,times(i),steps(i))
            call solutionsfile % close()
      ...
      if(associated(u_valuefun)) then
            ...
            ! allocate memory
            call fvl_allocate(u_exactvals,2,primal_numedges(i))

            ! compute exact values
            call primalmesh % evaledgemeanvalues(u_valuefun,u_exactvals)
            ...
            call solutionsfile % open()
            call solutionsfile % writedata(u_exactvals,fvl_trim(u_label)//" Exact",fvl_data_file_edgemeanvalues,times(i),steps(i))
            call solutionsfile % close()
            ...
            ! allocate memory
            call fvl_allocate(u_refvals2,2*primalrefmesh%getnumedges())

            ! reshape data
            u_refvals2 = reshape(transpose(u_refvals1),[2*primalrefmesh%getnumedges()],order=[1])
            ...
            ux_reconstmap = fvl_reconstmap2d(primal_patch,primalrefmesh,primalmeshlocator,controldictfile,"reconst_map",&
                  u_refvals2(1:primalrefmesh%getnumedges()))
            uy_reconstmap = fvl_reconstmap2d(primal_patch,primalrefmesh,primalmeshlocator,controldictfile,"reconst_map",&
                  u_refvals2(primalrefmesh%getnumedges()+1:2*primalrefmesh%getnumedges()))
            ...
            ! allocate memory
            call fvl_allocate(u_exactvals,2,primal_numedges(i))
            call fvl_allocate(auxv,2*primal_numedges(i))

            ! map reference solution
            call ux_reconstmap % mapedgemeanvalues(primalmesh,auxv(1:primal_numedges(i)))
            call uy_reconstmap % mapedgemeanvalues(primalmesh,auxv(primal_numedges(i)+1:2*primal_numedges(i)))
            u_exactvals = reshape(auxv,[2,primal_numedges(i)],order=[2,1])
      ...
      if(associated(u_valuefun)) then
            ...
            ! allocate memory
            call fvl_allocate(diamond_u_exactvals,2,diamond_numedges(i))

            ! compute exact values
            call diamondmesh % evaledgemeanvalues(u_valuefun,diamond_u_exactvals)
            ...
            solutionsfile = fvl_data_file(path=fvl_trim(u_suffix),form=errorsfilesform(i),action="write")
            call solutionsfile % open()
            call solutionsfile % writedata(diamond_u_exactvals,fvl_trim(u_label)//" Exact",fvl_data_file_edgemeanvalues,times(i),steps(i))
            call solutionsfile % close()
            ...
            ! allocate memory
            call fvl_allocate(diamond_u_refvals2,2*diamondrefmesh%getnumedges())

            ! reshape data
            diamond_u_refvals2 = reshape(transpose(diamond_u_refvals1),[2*diamondrefmesh%getnumedges()],order=[1])
            ...
            diamond_ux_reconstmap = fvl_reconstmap2d(diamond_patch,diamondrefmesh,diamondmeshlocator,controldictfile,"reconst_map",&
                  diamond_u_refvals2(1:diamondrefmesh%getnumedges()))
            diamond_uy_reconstmap = fvl_reconstmap2d(diamond_patch,diamondrefmesh,diamondmeshlocator,controldictfile,"reconst_map",&
                  diamond_u_refvals2(diamondrefmesh%getnumedges()+1:2*diamondrefmesh%getnumedges()))
            ...
            ! allocate memory
            call fvl_allocate(diamond_u_exactvals,2,diamond_numedges(i))
            call fvl_allocate(auxv,2*diamond_numedges(i))

            ! map reference solution
            call diamond_ux_reconstmap % mapedgemeanvalues(diamondmesh,auxv(1:diamond_numedges(i)))
            call diamond_uy_reconstmap % mapedgemeanvalues(diamondmesh,auxv(diamond_numedges(i)+1:2*diamond_numedges(i)))
            diamond_u_exactvals = reshape(auxv,[2,diamond_numedges(i)],order=[2,1])
      ...
      if(exist1) then
            ...
            ! allocate memory
            call fvl_allocate(t_errorvals,primal_numedges(i))
            ...
            t_error2(i) = 0.0d0
            t_errorinf(i) = 0.0d0
            do j=1,primal_numedges(i)
                  edgelength = primalmesh%getedgelength(j)
                  t_error1(i) = t_error1(i)+abs(t_errorvals(j))*edgelength
                  t_error2(i) = t_error2(i)+(t_errorvals(j)**2.0)*edgelength
                  t_errorinf(i) = max(t_errorinf(i),abs(t_errorvals(j)))
            end do
            t_error1(i) = t_error1(i)/primalmeshlengths(i)
            t_error2(i) = sqrt(t_error2(i)/primalmeshlengths(i))

            ! compute convergence orders
            if(i>1) then
                  if(convergence==1) then
                        if(primal_numedges(i-1)>0 .and. primal_numedges(i-1)/=primal_numedges(i) .and. t_errorinf(i)>0.0d0) then
                              t_order1(i) = 2.0d0*log(t_error1(i-1)/t_error1(i))&
                                    /log(fvl_real(primal_numedges(i))/fvl_real(primal_numedges(i-1)))
                              t_order2(i) = 2.0d0*log(t_error2(i-1)/t_error2(i))&
                                    /log(fvl_real(primal_numedges(i))/fvl_real(primal_numedges(i-1)))
                              t_orderinf(i) = 2.0d0*log(t_errorinf(i-1)/t_errorinf(i))&
                                    /log(fvl_real(primal_numedges(i))/fvl_real(primal_numedges(i-1)))
                        else
                              t_order1(i) = -1.0d0
                              t_order2(i) = -1.0d0
            ...
            solutionsfile = fvl_data_file(path=fvl_trim(t_suffix),form=errorsfilesform(i),action="write")
            call solutionsfile % open()
            call solutionsfile % writedata(t_errorvals,t_map,fvl_trim(t_label)//" Error",fvl_data_file_edgemeanvalues,times(i),steps(i))
            call solutionsfile % close()
      ...
      if(exist1) then
            ...
            ! allocate memory
            call fvl_allocate(p_errorvals,primal_numedges(i))
            call fvl_omp_clean(p_errorvals)

            ! fix constant
            if(p_fixconstant==1) then
                  call fvl_omp_sub(p_exactvals,fvl_omp_dot(p_exactvals,primalmesh%getedgelengths())/primalmeshlengths(i))
                  call fvl_omp_sub(p_approxvals,fvl_omp_dot(p_approxvals,primalmesh%getedgelengths())/primalmeshlengths(i))
            end if
            ...
            do j=1,primal_numedges(i)
                  edgelength = primalmesh%getedgelength(j)
                  p_error1(i) = p_error1(i)+abs(p_errorvals(j))*edgelength
                  p_error2(i) = p_error2(i)+(p_errorvals(j)**2.0)*edgelength
                  p_errorinf(i) = max(p_errorinf(i),abs(p_errorvals(j)))
            end do
            p_error1(i) = p_error1(i)/primalmeshlengths(i)
            p_error2(i) = sqrt(p_error2(i)/primalmeshlengths(i))

            ! compute convergence orders
            if(i>1) then
                  if(convergence==1) then
                        if(primal_numedges(i-1)>0 .and. primal_numedges(i-1)/=primal_numedges(i) .and. p_errorinf(i)>0.0d0) then
                              p_order1(i) = 2.0d0*log(p_error1(i-1)/p_error1(i))&
                                    /log(fvl_real(primal_numedges(i))/fvl_real(primal_numedges(i-1)))
                              p_order2(i) = 2.0d0*log(p_error2(i-1)/p_error2(i))&
                                    /log(fvl_real(primal_numedges(i))/fvl_real(primal_numedges(i-1)))
                              p_orderinf(i) = 2.0d0*log(p_errorinf(i-1)/p_errorinf(i))&
                                    /log(fvl_real(primal_numedges(i))/fvl_real(primal_numedges(i-1)))
                        else
                              p_order1(i) = -1.0d0
                              p_order2(i) = -1.0d0
            ...
            solutionsfile = fvl_data_file(path=fvl_trim(p_suffix),form=errorsfilesform(i),action="write")
            call solutionsfile % open()
            call solutionsfile % writedata(p_errorvals,p_map,fvl_trim(p_label)//" Error",fvl_data_file_edgemeanvalues,times(i),steps(i))
            call solutionsfile % close()
      ...
      if(exist1) then
            ...
            ! allocate memory
            call fvl_allocate(u_errorvals,2,primal_numedges(i))
            ...
            do j=1,primal_numedges(i)
                  edgelength = primalmesh%getedgelength(j)
                  ux_error1(i) = ux_error1(i)+abs(u_errorvals(1,j))*edgelength
                  ux_error2(i) = ux_error2(i)+(u_errorvals(1,j)**2.0)*edgelength
                  ux_errorinf(i) = max(ux_errorinf(i),abs(u_errorvals(1,j)))
                  uy_error1(i) = uy_error1(i)+abs(u_errorvals(2,j))*edgelength
                  uy_error2(i) = uy_error2(i)+(u_errorvals(2,j)**2.0)*edgelength
                  uy_errorinf(i) = max(uy_errorinf(i),abs(u_errorvals(2,j)))
                  aux=fvl_vectormag2(u_errorvals(:,j))
                  u_error1(i) = u_error1(i)+aux*edgelength
                  u_error2(i) = u_error2(i)+(aux**2.0)*edgelength
                  u_errorinf(i) = max(u_errorinf(i),aux)
            end do
            ux_error1(i) = ux_error1(i)/primalmeshlengths(i)
            ux_error2(i) = sqrt(ux_error2(i)/primalmeshlengths(i))
            uy_error1(i) = uy_error1(i)/primalmeshlengths(i)
            uy_error2(i) = sqrt(uy_error2(i)/primalmeshlengths(i))
            u_error1(i) = u_error1(i)/primalmeshlengths(i)
            u_error2(i) = sqrt(u_error2(i)/primalmeshlengths(i))

            ! compute convergence orders
            if(i>1) then
                  if(convergence==1) then
                        if(primal_numedges(i-1)>0 .and. primal_numedges(i-1)/=primal_numedges(i)) then
                              if(ux_errorinf(i)>0.0d0) then
                                    ux_order1(i) = 2.0d0*log(ux_error1(i-1)/ux_error1(i))&
                                          /log(fvl_real(primal_numedges(i))/fvl_real(primal_numedges(i-1)))
                                    ux_order2(i) = 2.0d0*log(ux_error2(i-1)/ux_error2(i))&
                                          /log(fvl_real(primal_numedges(i))/fvl_real(primal_numedges(i-1)))
                                    ux_orderinf(i) = 2.0d0*log(ux_errorinf(i-1)/ux_errorinf(i))&
                                          /log(fvl_real(primal_numedges(i))/fvl_real(primal_numedges(i-1)))
                              else
                                    ux_order1(i) = -1.0d0
                                    ux_order2(i) = -1.0d0
                              ...
                              if(uy_errorinf(i)>0.0d0) then
                                    uy_order1(i) = 2.0d0*log(uy_error1(i-1)/uy_error1(i))&
                                          /log(fvl_real(primal_numedges(i))/fvl_real(primal_numedges(i-1)))
                                    uy_order2(i) = 2.0d0*log(uy_error2(i-1)/uy_error2(i))&
                                          /log(fvl_real(primal_numedges(i))/fvl_real(primal_numedges(i-1)))
                                    uy_orderinf(i) = 2.0d0*log(uy_errorinf(i-1)/uy_errorinf(i))&
                                          /log(fvl_real(primal_numedges(i))/fvl_real(primal_numedges(i-1)))
                              else
                                    uy_order1(i) = -1.0d0
                                    uy_order2(i) = -1.0d0
                              ...
                              if(u_errorinf(i)>0.0d0) then
                                    u_order1(i) = 2.0d0*log(u_error1(i-1)/u_error1(i))&
                                          /log(fvl_real(primal_numedges(i))/fvl_real(primal_numedges(i-1)))
                                    u_order2(i) = 2.0d0*log(u_error2(i-1)/u_error2(i))&
                                          /log(fvl_real(primal_numedges(i))/fvl_real(primal_numedges(i-1)))
                                    u_orderinf(i) = 2.0d0*log(u_errorinf(i-1)/u_errorinf(i))&
                                          /log(fvl_real(primal_numedges(i))/fvl_real(primal_numedges(i-1)))
                              else
                                    u_order1(i) = -1.0d0
                                    u_order2(i) = -1.0d0
            ...
            solutionsfile = fvl_data_file(path=fvl_trim(u_suffix),form=errorsfilesform(i),action="write")
            call solutionsfile % open()
            call solutionsfile % writedata(u_errorvals,u_map,fvl_trim(u_label)//" Error",fvl_data_file_edgemeanvalues,times(i),steps(i))
            call solutionsfile % close()
      ...
      if(exist1) then
            ...
            ! allocate memory
            call fvl_allocate(diamond_u_errorvals,2,diamond_numedges(i))
            ...
            do j=1,diamond_numedges(i)
                  edgelength = diamondmesh%getedgelength(j)
                  diamond_ux_error1(i) = diamond_ux_error1(i)+abs(diamond_u_errorvals(1,j))*edgelength
                  diamond_ux_error2(i) = diamond_ux_error2(i)+(diamond_u_errorvals(1,j)**2.0)*edgelength
                  diamond_ux_errorinf(i) = max(diamond_ux_errorinf(i),abs(diamond_u_errorvals(1,j)))
                  diamond_uy_error1(i) = diamond_uy_error1(i)+abs(diamond_u_errorvals(2,j))*edgelength
                  diamond_uy_error2(i) = diamond_uy_error2(i)+(diamond_u_errorvals(2,j)**2.0)*edgelength
                  diamond_uy_errorinf(i) = max(diamond_uy_errorinf(i),abs(diamond_u_errorvals(2,j)))
                  aux=fvl_vectormag2(diamond_u_errorvals(:,j))
                  diamond_u_error1(i) = diamond_u_error1(i)+aux*edgelength
                  diamond_u_error2(i) = diamond_u_error2(i)+(aux**2.0)*edgelength
                  diamond_u_errorinf(i) = max(diamond_u_errorinf(i),aux)
            end do
            diamond_ux_error1(i) = diamond_ux_error1(i)/diamondmeshlengths(i)
            diamond_ux_error2(i) = sqrt(diamond_ux_error2(i)/diamondmeshlengths(i))
            diamond_uy_error1(i) = diamond_uy_error1(i)/diamondmeshlengths(i)
            diamond_uy_error2(i) = sqrt(diamond_uy_error2(i)/diamondmeshlengths(i))
            diamond_u_error1(i) = diamond_u_error1(i)/diamondmeshlengths(i)
            diamond_u_error2(i) = sqrt(diamond_u_error2(i)/diamondmeshlengths(i))

            ! compute convergence orders
            if(i>1) then
                  if(convergence==1) then
                        if(diamond_numedges(i-1)>0 .and. diamond_numedges(i-1)/=diamond_numedges(i)) then
                              if(diamond_ux_errorinf(i)>0.0d0) then
                                    diamond_ux_order1(i) = 2.0d0*log(diamond_ux_error1(i-1)/diamond_ux_error1(i))&
                                          /log(fvl_real(diamond_numedges(i))/fvl_real(diamond_numedges(i-1)))
                                    diamond_ux_order2(i) = 2.0d0*log(diamond_ux_error2(i-1)/diamond_ux_error2(i))&
                                          /log(fvl_real(diamond_numedges(i))/fvl_real(diamond_numedges(i-1)))
                                    diamond_ux_orderinf(i) = 2.0d0*log(diamond_ux_errorinf(i-1)/diamond_ux_errorinf(i))&
                                          /log(fvl_real(diamond_numedges(i))/fvl_real(diamond_numedges(i-1)))
                              else
                                    diamond_ux_order1(i) = -1.0d0
                                    diamond_ux_order2(i) = -1.0d0
                              ...
                              if(diamond_uy_errorinf(i)>0.0d0) then
                                    diamond_uy_order1(i) = 2.0d0*log(diamond_uy_error1(i-1)/diamond_uy_error1(i))&
                                          /log(fvl_real(diamond_numedges(i))/fvl_real(diamond_numedges(i-1)))
                                    diamond_uy_order2(i) = 2.0d0*log(diamond_uy_error2(i-1)/diamond_uy_error2(i))&
                                          /log(fvl_real(diamond_numedges(i))/fvl_real(diamond_numedges(i-1)))
                                    diamond_uy_orderinf(i) = 2.0d0*log(diamond_uy_errorinf(i-1)/diamond_uy_errorinf(i))&
                                          /log(fvl_real(diamond_numedges(i))/fvl_real(diamond_numedges(i-1)))
                              else
                                    diamond_uy_order1(i) = -1.0d0
                                    diamond_uy_order2(i) = -1.0d0
                              ...
                              if(diamond_u_errorinf(i)>0.0d0) then
                                    diamond_u_order1(i) = 2.0d0*log(diamond_u_error1(i-1)/diamond_u_error1(i))&
                                          /log(fvl_real(diamond_numedges(i))/fvl_real(diamond_numedges(i-1)))
                                    diamond_u_order2(i) = 2.0d0*log(diamond_u_error2(i-1)/diamond_u_error2(i))&
                                          /log(fvl_real(diamond_numedges(i))/fvl_real(diamond_numedges(i-1)))
                                    diamond_u_orderinf(i) = 2.0d0*log(diamond_u_errorinf(i-1)/diamond_u_errorinf(i))&
                                          /log(fvl_real(diamond_numedges(i))/fvl_real(diamond_numedges(i-1)))
                              else
                                    diamond_u_order1(i) = -1.0d0
                                    diamond_u_order2(i) = -1.0d0
            ...
            solutionsfile = fvl_data_file(path=fvl_trim(u_suffix),form=errorsfilesform(i),action="write")
            call solutionsfile % open()
            call solutionsfile % writedata(diamond_u_errorvals,diamond_u_map,fvl_trim(u_label)//" Error",fvl_data_file_edgemeanvalues,times(i),steps(i))
            call solutionsfile % close()
...
! dofs
if(convergence==1) then
      primal_numdofs(1:nummeshes) = primal_numedges(1:nummeshes)
      diamond_numdofs(1:nummeshes) = diamond_numedges(1:nummeshes)
