...
!|  _ _ _   _   _   _      |                                   |
!| |_|_|_| |_| |_| |_|     |  The Great Finite area Library  |
...
! Modification: November, 2023

#include "macros.f"

program fvl_post2_thermoincompflow2d_main

use fvl_lib2d

use fvl_meshes2d_mod
...
character(len=__fvl_character_len__)::postdictpath,t_suffix,p_suffix,u_suffix,&
      t_label,p_label,u_label,intformat,realformat,primal_doflabel,diamond_doflabel,&
...
integer(kind=__fvl_integer_kind__)::i,j,k,nummeshes,convergence,p_fixconstant,tablesprecision,unit,element,&
      primal_numdofs(maxnummeshes),diamond_numdofs(maxnummeshes),&
      primal_numboundpoints(maxnummeshes),diamond_numboundpoints(maxnummeshes),steps(maxnummeshes),&
      linearsystemnumiters(maxnummeshes),fixpointnumiters(maxnummeshes)
real(kind=__fvl_real_kind__)::time,aux,&
...
type(fvl_mesh2d)::primalmesh,diamondmesh,primalrefmesh,diamondrefmesh
type(fvl_file)::file
type(fvl_keys_file)::paramfile
type(fvl_data_file)::solutionsfile
type(fvl_dict_file)::controldictfile
type(fvl_patch2d)::primal_patch,diamond_patch
! type(fvl_meshlocator2d)::primalmeshlocator,diamondmeshlocator
! type(fvl_reconstmap2d)::t_reconstmap,p_reconstmap,ux_reconstmap,uy_reconstmap,diamond_ux_reconstmap,diamond_uy_reconstmap
...
if(fvl_findopt("-h") .or. fvl_findopt("--help")) then
      ...
      call fvl_loginfo("Usage: fvl-postbound-thermoincompflow2d [options]")
...
do i=1,maxnummeshes
      ...
      if(exist1) then
            ...
            ! read mesh
            primalmesh = fvl_mesh2d(filepath=meshesfilepaths(i),fileform=meshesfileforms(i),dictfile=controldictfile,dictlabel="mesh")
            ...
            ! create diamond mesh
            diamondmesh = fvl_diamond_mesh2d(primalmesh,dictfile=controldictfile,dictlabel="mesh")

            ! write mesh report
            call diamondmesh % report("- diamond mesh")

            ! initialize variables
            primal_numboundpoints(i) = fvl_omp_sum(primalmesh%getnumedgeboundpoints())
            diamond_numboundpoints(i) = fvl_omp_sum(diamondmesh%getnumedgeboundpoints())
            ...
            ! initialize variables
            primal_numboundpoints(i) = 0
            diamond_numboundpoints(i) = 0
            t_error1(i) = -1.0d0
            t_error2(i) = -1.0d0
            t_errorinf(i) = -1.0d0
            t_order1(i) = -1.0d0
      ...
      if(exist1 .and. (.not. associated(t_valuefun) .or. .not. associated(p_valuefun) .or. .not. associated(u_valuefun))) then
            ...
            ! read mesh
            primalrefmesh = fvl_mesh2d(filepath=refmeshesfilepaths(i),fileform=refmeshesfileforms(i),quadratorder=8)
            ...
            ! create diamond mesh
            diamondrefmesh = fvl_diamond_mesh2d(primalrefmesh,quadratorder=8)
            ...
            ! ! mesh locator
            ! primalmeshlocator = fvl_meshlocator2d(primalrefmesh,controldictfile,"mesh_locator")
            ! diamondmeshlocator = fvl_meshlocator2d(diamondrefmesh,controldictfile,"mesh_locator")

            ! mesh patches
            primal_patch = fvl_patch2d(primalrefmesh)
            diamond_patch = fvl_patch2d(diamondrefmesh)

            ! ! state message
            ! call fvl_loginfo("  >>> Computing reconstruction mappers")
            !
            ! reconstruction map
            ! primal_reconstmap = fvl_reconstmap2d(primal_patch,primalrefmesh,primalmeshlocator,controldictfile,"reconst_map")
            ! diamond_reconstmap = fvl_reconstmap2d(diamond_patch,diamondrefmesh,diamondmeshlocator,controldictfile,"reconst_map")
...
#include "models.f"
      ...
      if(fvl_trim(t_label)/=fvl_char_empty) then
            if(fvl_trim(refsolutionsfilesform(i))=="ascii") then
                  t_suffix = fvl_trim(refsolutionsfilesdir(i))//"/"//fvl_trim(t_label)//".fvd2"
            else
                  t_suffix = fvl_trim(refsolutionsfilesdir(i))//"/"//fvl_trim(t_label)//".fvd2.bin"
            end if
      else
            go to 3001
      end if
      ...
      if(associated(t_valuefun)) then
            ...
            ! allocate memory
            call fvl_allocate(t_exactvals,primalmesh%getmaxnumboundpointsperedge(),primalmesh%getnumboundedges())

            ! compute exact values
            call primalmesh % evalboundpointvalues(t_valuefun,t_exactvals)
            ...
            if(fvl_trim(solutionsfilesform(i))=="ascii") then
                  t_suffix = fvl_trim(solutionsfilesdir(i))//"/"//fvl_trim(t_label)//"_exact.fvd2"
            else
                  t_suffix = fvl_trim(solutionsfilesdir(i))//"/"//fvl_trim(t_label)//"_exact.fvd2.bin"
            end if

            ! write file
            solutionsfile = fvl_data_file(path=fvl_trim(t_suffix),form=errorsfilesform(i),action="write")
            call solutionsfile % open()
            call solutionsfile % writedata(t_exactvals,fvl_trim(t_label)//" Exact",fvl_data_file_boundpointvalues,times(i),steps(i))
      ...
      !       call solutionsfile % open()
      !       element = 0
      !       do while(element/=fvl_data_file_boundpointvalues)
      ...
      !
      !       ! reconstruction map
      !       t_reconstmap = fvl_reconstmap2d(primal_patch,primalrefmesh,primalmeshlocator,controldictfile,"reconst_map",t_refvals)
      !
      !       ! state message
      !       3000 continue
      !       call fvl_loginfo("  >>> Computing exact temperature solution")
      !
      !       ! allocate memory
      !       call fvl_allocate(t_exactvals,primal_numboundpoints(i))
      !
      !       ! map reference solution
      !       call t_reconstmap % mapedgemeanvalues(primalmesh,t_exactvals)
      !
      !       ! state message
      !       call fvl_loginfo("  >>> Writing exact temperature solution")
      !
      ...
      !       call solutionsfile % close()
            ...
            ! allocate memory
            call fvl_allocate(t_exactvals,primalmesh%getmaxnumboundpointsperedge(),primal_numboundpoints(i))
      ...
      if(fvl_trim(p_label)/=fvl_char_empty) then
            if(fvl_trim(refsolutionsfilesform(i))=="ascii") then
                  p_suffix = fvl_trim(refsolutionsfilesdir(i))//"/"//fvl_trim(p_label)//".fvd2"
            else
                  p_suffix = fvl_trim(refsolutionsfilesdir(i))//"/"//fvl_trim(p_label)//".fvd2.bin"
            end if
      else
            go to 4001
      end if
      ...
      if(associated(p_valuefun)) then
            ...
            ! allocate memory
            call fvl_allocate(p_exactvals,primalmesh%getmaxnumboundpointsperedge(),primalmesh%getnumboundedges())

            ! compute exact values
            call primalmesh % evalboundpointvalues(p_valuefun,p_exactvals)
            ...
            ! suffixes
            if(fvl_trim(solutionsfilesform(i))=="ascii") then
                  p_suffix = fvl_trim(solutionsfilesdir(i))//"/"//fvl_trim(p_label)//"_exact.fvd2"
            else
                  p_suffix = fvl_trim(solutionsfilesdir(i))//"/"//fvl_trim(p_label)//"_exact.fvd2.bin"
            end if

            ! write file
            solutionsfile = fvl_data_file(path=fvl_trim(p_suffix),form=errorsfilesform(i),action="write")
            call solutionsfile % open()
            call solutionsfile % writedata(p_exactvals,fvl_trim(p_label)//" Exact",fvl_data_file_boundpointvalues,times(i),steps(i))
            call solutionsfile % close()
      ...
      !
      !       ! reconstruction map
      !       p_reconstmap = fvl_reconstmap2d(primal_patch,primalrefmesh,primalmeshlocator,controldictfile,"reconst_map",p_refvals)
      !
      !       ! state message
      !       4000 continue
      !       call fvl_loginfo("  >>> Computing exact pressure solution")
      !
      !       ! allocate memory
      !       call fvl_allocate(p_exactvals,primal_numboundpoints(i))
      !
      !       ! map reference solution
      !       call p_reconstmap % mapedgemeanvalues(primalmesh,p_exactvals)
      !
      !       ! state message
      !       call fvl_loginfo("  >>> Writing exact pressure solution")
      !
      ...
      !       call solutionsfile % close()
            ...
            ! allocate memory
            call fvl_allocate(p_exactvals,primalmesh%getmaxnumboundpointsperedge(),primal_numboundpoints(i))
      ...
      if(fvl_trim(u_label)/=fvl_char_empty) then
            if(refsolutionsfilesform(i)=="ascii") then
                  u_suffix = fvl_trim(refsolutionsfilesdir(i))//"/"//fvl_trim(u_label)//".fvd2"
            else
                  u_suffix = fvl_trim(refsolutionsfilesdir(i))//"/"//fvl_trim(u_label)//".fvd2.bin"
            end if
      else
            go to 5001
      end if
      ...
      if(associated(u_valuefun)) then
            ...
            ! allocate memory
            call fvl_allocate(u_exactvals,2,primalmesh%getmaxnumboundpointsperedge(),primalmesh%getnumboundedges())

            ! compute exact values
            call primalmesh % evalboundpointvalues(u_valuefun,u_exactvals)
            ...
            ! suffixes
            if(fvl_trim(solutionsfilesform(i))=="ascii") then
                  u_suffix = fvl_trim(solutionsfilesdir(i))//"/"//fvl_trim(u_label)//"_exact.fvd2"
            else
                  u_suffix = fvl_trim(solutionsfilesdir(i))//"/"//fvl_trim(u_label)//"_exact.fvd2.bin"
            end if

            ! write file
            solutionsfile = fvl_data_file(path=fvl_trim(u_suffix),form=errorsfilesform(i),action="write")
            call solutionsfile % open()
            call solutionsfile % writedata(u_exactvals,fvl_trim(u_label)//" Exact",fvl_data_file_boundpointvalues,times(i),steps(i))
            call solutionsfile % close()
      ...
      !       element = 0
      !       do while(element/=fvl_data_file_boundpointvalues)
      !             call solutionsfile % readdata(res=u_refvals1,map=u_map,element=element,time=times(i),step=steps(i))
      !       end do
      !       call solutionsfile % close()
      !
      !       ! check if data exists
      ...
      !
      !       ! allocate memory
      !       call fvl_allocate(u_refvals2,2*primalrefmesh%getnumcells())
      !
      !       ! reshape data
      !       u_refvals2 = reshape(transpose(u_refvals1),[2*primalrefmesh%getnumcells()],order=[1])
      !
      !       ! state message
      !       call fvl_loginfo("  >>> Computing velocity reconstruction map")
      !
      !       ! reconstruction map
      !       ux_reconstmap = fvl_reconstmap2d(primal_patch,primalrefmesh,primalmeshlocator,controldictfile,"reconst_map",&
      !             u_refvals2(1:primalrefmesh%getnumcells()))
      !       uy_reconstmap = fvl_reconstmap2d(primal_patch,primalrefmesh,primalmeshlocator,controldictfile,"reconst_map",&
      ...
      !
      !       ! allocate memory
      !       call fvl_allocate(u_exactvals,2,primal_numboundpoints(i))
      !       call fvl_allocate(auxv,2*primal_numboundpoints(i))
      !
      !       ! map reference solution
      !       call ux_reconstmap % mapedgemeanvalues(primalmesh,auxv(1:primal_numboundpoints(i)))
      !       call uy_reconstmap % mapedgemeanvalues(primalmesh,auxv(primal_numboundpoints(i)+1:2*primal_numboundpoints(i)))
      !       u_exactvals = reshape(auxv,[2,primal_numboundpoints(i)],order=[2,1])
      !
      !       ! state message
      !       call fvl_loginfo("  >>> Writing exact velocity solution")
      !
      ...
      !       call solutionsfile % close()
            ...
            ! allocate memory
            call fvl_allocate(u_exactvals,2,primalmesh%getmaxnumboundpointsperedge(),primal_numboundpoints(i))
      ...
      if(fvl_trim(u_label)/=fvl_char_empty) then
            if(fvl_trim(refsolutionsfilesform(i))=="ascii") then
                  u_suffix = fvl_trim(refsolutionsfilesdir(i))//"/"//fvl_trim(u_label)//"_diamond.fvd2"
            else
                  u_suffix = fvl_trim(refsolutionsfilesdir(i))//"/"//fvl_trim(u_label)//"_diamond.fvd2.bin"
            end if
      else
            go to 6001
      end if
      ...
      if(associated(u_valuefun)) then
            ...
            ! allocate memory
            call fvl_allocate(diamond_u_exactvals,2,diamondmesh%getmaxnumboundpointsperedge(),diamondmesh%getnumboundedges())

            ! compute exact values
            call diamondmesh % evalboundpointvalues(u_valuefun,diamond_u_exactvals)
            ...
            ! suffixes
            if(fvl_trim(solutionsfilesform(i))=="ascii") then
                  u_suffix = fvl_trim(solutionsfilesdir(i))//"/"//fvl_trim(u_label)//"_diamond_exact.fvd2"
            else
                  u_suffix = fvl_trim(solutionsfilesdir(i))//"/"//fvl_trim(u_label)//"_diamond_exact.fvd2.bin"
            end if

            ! write file
            solutionsfile = fvl_data_file(path=fvl_trim(u_suffix),form=errorsfilesform(i),action="write")
            call solutionsfile % open()
            call solutionsfile % writedata(diamond_u_exactvals,fvl_trim(u_label)//" Exact",fvl_data_file_boundpointvalues,times(i),steps(i))
            call solutionsfile % close()
      ...
      !       element = 0
      !       do while(element/=fvl_data_file_boundpointvalues)
      !             call solutionsfile % readdata(res=diamond_u_refvals1,map=diamond_u_map,element=element,time=times(i),step=steps(i))
      !       end do
      !       call solutionsfile % close()
      !
      !       ! check if data exists
      ...
      !
      !       ! allocate memory
      !       call fvl_allocate(diamond_u_refvals2,2*diamondrefmesh%getnumcells())
      !
      !       ! reshape data
      !       diamond_u_refvals2 = reshape(transpose(diamond_u_refvals1),[2*diamondrefmesh%getnumcells()],order=[1])
      !
      !       ! state message
      !       call fvl_loginfo("  >>> Computing diamond velocity reconstruction map")
      !
      !       ! reconstruction map
      !       diamond_ux_reconstmap = fvl_reconstmap2d(diamond_patch,diamondrefmesh,diamondmeshlocator,controldictfile,"reconst_map",&
      !             diamond_u_refvals2(1:diamondrefmesh%getnumcells()))
      !       diamond_uy_reconstmap = fvl_reconstmap2d(diamond_patch,diamondrefmesh,diamondmeshlocator,controldictfile,"reconst_map",&
      ...
      !
      !       ! allocate memory
      !       call fvl_allocate(diamond_u_exactvals,2,diamond_numboundpoints(i))
      !       call fvl_allocate(auxv,2*diamond_numboundpoints(i))
      !
      !       ! map reference solution
      !       call diamond_ux_reconstmap % mapedgemeanvalues(diamondmesh,auxv(1:diamond_numboundpoints(i)))
      !       call diamond_uy_reconstmap % mapedgemeanvalues(diamondmesh,auxv(diamond_numboundpoints(i)+1:2*diamond_numboundpoints(i)))
      !       diamond_u_exactvals = reshape(auxv,[2,diamond_numboundpoints(i)],order=[2,1])
      !
      !       ! state message
      !       call fvl_loginfo("  >>> Writing exact diamond velocity solution")
      !
      ...
      !       call solutionsfile % close()
            ...
            ! allocate memory
            call fvl_allocate(diamond_u_exactvals,2,diamondmesh%getmaxnumboundpointsperedge(),diamond_numboundpoints(i))
      ...
      if(fvl_trim(t_label)/=fvl_char_empty) then
            if(fvl_trim(solutionsfilesform(i))=="ascii") then
                  t_suffix = fvl_trim(solutionsfilesdir(i))//"/"//fvl_trim(t_label)//".fvd2"
            else
                  t_suffix = fvl_trim(solutionsfilesdir(i))//"/"//fvl_trim(t_label)//".fvd2.bin"
            end if
      else
            go to 7000
      end if
      ...
      if(exist1) then
            ...
            ! allocate memory
            call fvl_allocate(t_errorvals,primalmesh%getmaxnumboundpointsperedge(),primalmesh%getnumboundedges())
            ...
            t_error2(i) = 0.0d0
            t_errorinf(i) = 0.0d0
            do j=1,primalmesh%getnumboundedges()
                  do k=1,primalmesh%getnumedgeboundpoints(j)
                        t_error1(i) = t_error1(i)+abs(t_errorvals(k,j))
                        t_error2(i) = t_error2(i)+(t_errorvals(k,j)**2.0)
                        t_errorinf(i) = max(t_errorinf(i),abs(t_errorvals(k,j)))
                  end do
            end do
            t_error1(i) = t_error1(i)/fvl_real(primal_numboundpoints(i))
            t_error2(i) = sqrt(t_error2(i)/fvl_real(primal_numboundpoints(i)))

            ! compute convergence orders
            if(i>1) then
                  if(convergence==1) then
                        if(primal_numboundpoints(i-1)>0 .and. primal_numboundpoints(i-1)/=primal_numboundpoints(i) .and. t_errorinf(i)>0.0d0) then
                              t_order1(i) = log(t_error1(i-1)/t_error1(i))&
                                    /log(fvl_real(primal_numboundpoints(i))/fvl_real(primal_numboundpoints(i-1)))
            ...
            ! suffixes
            if(fvl_trim(solutionsfilesform(i))=="ascii") then
                  t_suffix = fvl_trim(solutionsfilesdir(i))//"/"//fvl_trim(t_label)//"_error.fvd2"
            else
                  t_suffix = fvl_trim(solutionsfilesdir(i))//"/"//fvl_trim(t_label)//"_error.fvd2.bin"
            end if

            ! write file
            solutionsfile = fvl_data_file(path=fvl_trim(t_suffix),form=errorsfilesform(i),action="write")
            call solutionsfile % open()
            call solutionsfile % writedata(t_errorvals,t_map,fvl_trim(t_label)//" Error",fvl_data_file_boundpointvalues,times(i),steps(i))
            call solutionsfile % close()
      ...
      if(fvl_trim(p_label)/=fvl_char_empty) then
            if(fvl_trim(solutionsfilesform(i))=="ascii") then
                  p_suffix = fvl_trim(solutionsfilesdir(i))//"/"//fvl_trim(p_label)//".fvd2"
            else
                  p_suffix = fvl_trim(solutionsfilesdir(i))//"/"//fvl_trim(p_label)//".fvd2.bin"
            end if
      else
            go to 8000
      end if
      ...
      if(exist1) then
            ...
            ! allocate memory
            call fvl_allocate(p_errorvals,primalmesh%getmaxnumboundpointsperedge(),primalmesh%getnumboundedges())
            ...
            do j=1,primalmesh%getnumboundedges()
                  do k=1,primalmesh%getnumedgeboundpoints(j)
                        p_error1(i) = p_error1(i)+abs(p_errorvals(k,j))
                        p_error2(i) = p_error2(i)+(p_errorvals(k,j)**2.0)
                        p_errorinf(i) = max(p_errorinf(i),abs(p_errorvals(k,j)))
                  end do
            end do
            p_error1(i) = p_error1(i)/fvl_real(primal_numboundpoints(i))
            p_error2(i) = sqrt(p_error2(i)/fvl_real(primal_numboundpoints(i)))

            ! compute convergence orders
            if(i>1) then
                  if(convergence==1) then
                        if(primal_numboundpoints(i-1)>0 .and. primal_numboundpoints(i-1)/=primal_numboundpoints(i) .and. p_errorinf(i)>0.0d0) then
            ...
            ! suffixes
            if(fvl_trim(solutionsfilesform(i))=="ascii") then
                  p_suffix = fvl_trim(solutionsfilesdir(i))//"/"//fvl_trim(p_label)//"_error.fvd2"
            else
                  p_suffix = fvl_trim(solutionsfilesdir(i))//"/"//fvl_trim(p_label)//"_error.fvd2.bin"
            end if

            ! write file
            solutionsfile = fvl_data_file(path=fvl_trim(p_suffix),form=errorsfilesform(i),action="write")
            call solutionsfile % open()
            call solutionsfile % writedata(p_errorvals,p_map,fvl_trim(p_label)//" Error",fvl_data_file_boundpointvalues,times(i),steps(i))
            call solutionsfile % close()
      ...
      if(fvl_trim(u_label)/=fvl_char_empty) then
            if(fvl_trim(solutionsfilesform(i))=="ascii") then
                  u_suffix = fvl_trim(solutionsfilesdir(i))//"/"//fvl_trim(u_label)//".fvd2"
            else
                  u_suffix = fvl_trim(solutionsfilesdir(i))//"/"//fvl_trim(u_label)//".fvd2.bin"
            end if
      else
            go to 9000
      end if
      ...
      if(exist1) then
            ...
            ! allocate memory
            call fvl_allocate(u_errorvals,2,primalmesh%getmaxnumboundpointsperedge(),primalmesh%getnumboundedges())
            ...
            do j=1,primalmesh%getnumboundedges()
                  do k=1,primalmesh%getnumedgeboundpoints(j)
                        ux_error1(i) = ux_error1(i)+abs(u_errorvals(1,k,j))
                        ux_error2(i) = ux_error2(i)+(u_errorvals(1,k,j)**2.0)
                        ux_errorinf(i) = max(ux_errorinf(i),abs(u_errorvals(1,k,j)))
                        uy_error1(i) = uy_error1(i)+abs(u_errorvals(2,k,j))
                        uy_error2(i) = uy_error2(i)+(u_errorvals(2,k,j)**2.0)
                        uy_errorinf(i) = max(uy_errorinf(i),abs(u_errorvals(2,k,j)))
                        aux=fvl_vectormag2(u_errorvals(:,k,j))
                        u_error1(i) = u_error1(i)+aux
                        u_error2(i) = u_error2(i)+(aux**2.0)
                        u_errorinf(i) = max(u_errorinf(i),aux)
                  end do
            end do
            ux_error1(i) = ux_error1(i)/fvl_real(primal_numboundpoints(i))
            ux_error2(i) = sqrt(ux_error2(i)/fvl_real(primal_numboundpoints(i)))
            uy_error1(i) = uy_error1(i)/fvl_real(primal_numboundpoints(i))
            uy_error2(i) = sqrt(uy_error2(i)/fvl_real(primal_numboundpoints(i)))
            u_error1(i) = u_error1(i)/fvl_real(primal_numboundpoints(i))
            u_error2(i) = sqrt(u_error2(i)/fvl_real(primal_numboundpoints(i)))

            ! compute convergence orders
            if(i>1) then
                  if(convergence==1) then
                        if(primal_numboundpoints(i-1)>0 .and. primal_numboundpoints(i-1)/=primal_numboundpoints(i)) then
            ...
            ! suffixes
            if(fvl_trim(solutionsfilesform(i))=="ascii") then
                  u_suffix = fvl_trim(solutionsfilesdir(i))//"/"//fvl_trim(u_label)//"_error.fvd2"
            else
                  u_suffix = fvl_trim(solutionsfilesdir(i))//"/"//fvl_trim(u_label)//"_error.fvd2.bin"
            end if

            ! write file
            solutionsfile = fvl_data_file(path=fvl_trim(u_suffix),form=errorsfilesform(i),action="write")
            call solutionsfile % open()
            call solutionsfile % writedata(u_errorvals,u_map,fvl_trim(u_label)//" Error",fvl_data_file_boundpointvalues,times(i),steps(i))
            call solutionsfile % close()
      ...
      if(fvl_trim(u_label)/=fvl_char_empty) then
            if(fvl_trim(solutionsfilesform(i))=="ascii") then
                  u_suffix = fvl_trim(solutionsfilesdir(i))//"/"//fvl_trim(u_label)//"_diamond.fvd2"
            else
                  u_suffix = fvl_trim(solutionsfilesdir(i))//"/"//fvl_trim(u_label)//"_diamond.fvd2.bin"
            end if
      else
            go to 10000
      end if
      ...
      if(exist1) then
            ...
            ! allocate memory
            call fvl_allocate(diamond_u_errorvals,2,primalmesh%getmaxnumboundpointsperedge(),primalmesh%getnumboundedges())
            call fvl_omp_clean(diamond_u_errorvals)

            ! compute errors
            do j=1,size(diamond_u_map,1)
                  diamond_u_errorvals(:,:,u_map(j)) = diamond_u_approxvals(:,:,j)-diamond_u_exactvals(:,:,u_map(j))
            end do
            ...
            do j=1,diamondmesh%getnumboundedges()
                  do k=1,diamondmesh%getnumedgeboundpoints(j)
                        diamond_ux_error1(i) = diamond_ux_error1(i)+abs(diamond_u_errorvals(1,k,j))
                        diamond_ux_error2(i) = diamond_ux_error2(i)+(diamond_u_errorvals(1,k,j)**2.0)
                        diamond_ux_errorinf(i) = max(diamond_ux_errorinf(i),abs(diamond_u_errorvals(1,k,j)))
                        diamond_uy_error1(i) = diamond_uy_error1(i)+abs(diamond_u_errorvals(2,k,j))
                        diamond_uy_error2(i) = diamond_uy_error2(i)+(diamond_u_errorvals(2,k,j)**2.0)
                        diamond_uy_errorinf(i) = max(diamond_uy_errorinf(i),abs(diamond_u_errorvals(2,k,j)))
                        aux=fvl_vectormag2(diamond_u_errorvals(:,k,j))
                        diamond_u_error1(i) = diamond_u_error1(i)+aux
                        diamond_u_error2(i) = diamond_u_error2(i)+(aux**2.0)
                        diamond_u_errorinf(i) = max(diamond_u_errorinf(i),aux)
                  end do
            end do
            diamond_ux_error1(i) = diamond_ux_error1(i)/fvl_real(diamond_numboundpoints(i))
            diamond_ux_error2(i) = sqrt(diamond_ux_error2(i)/fvl_real(diamond_numboundpoints(i)))
            diamond_uy_error1(i) = diamond_uy_error1(i)/fvl_real(diamond_numboundpoints(i))
            diamond_uy_error2(i) = sqrt(diamond_uy_error2(i)/fvl_real(diamond_numboundpoints(i)))
            diamond_u_error1(i) = diamond_u_error1(i)/fvl_real(diamond_numboundpoints(i))
            diamond_u_error2(i) = sqrt(diamond_u_error2(i)/fvl_real(diamond_numboundpoints(i)))

            ! compute convergence orders
            if(i>1) then
                  if(convergence==1) then
                        if(diamond_numboundpoints(i-1)>0 .and. diamond_numboundpoints(i-1)/=diamond_numboundpoints(i)) then
                              if(diamond_ux_errorinf(i)>0.0d0) then
                                    diamond_ux_order1(i) = log(diamond_ux_error1(i-1)/diamond_ux_error1(i))&
                                          /log(fvl_real(diamond_numboundpoints(i))/fvl_real(diamond_numboundpoints(i-1)))
            ...
            ! suffixes
            if(fvl_trim(solutionsfilesform(i))=="ascii") then
                  u_suffix = fvl_trim(solutionsfilesdir(i))//"/"//fvl_trim(u_label)//"_diamond_error.fvd2"
            else
                  u_suffix = fvl_trim(solutionsfilesdir(i))//"/"//fvl_trim(u_label)//"_diamond_error.fvd2.bin"
            end if

            ! write file
            solutionsfile = fvl_data_file(path=fvl_trim(u_suffix),form=errorsfilesform(i),action="write")
            call solutionsfile % open()
            call solutionsfile % writedata(diamond_u_errorvals,u_map,fvl_trim(u_label)//" Error",fvl_data_file_boundpointvalues,times(i),steps(i))
            call solutionsfile % close()
      ...
      ! inspect file
      inquire(file=fvl_trim(paramsfilesdir(i))//"/$1 & & $2 & & $3 & & $4 \\stem.fvd",exist=exist1)
...
nummeshes         = 0
do i=1,maxnummeshes
      if(primal_numboundpoints(i)/=0) then
            nummeshes = i
      end if
end do
...
! dofs
if(convergence==1) then
      primal_numdofs(1:nummeshes) = primal_numboundpoints(1:nummeshes)
      diamond_numdofs(1:nummeshes) = diamond_numboundpoints(1:nummeshes)
...
end program fvl_post2_thermoincompflow2d_main
! end of file
