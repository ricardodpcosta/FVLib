...
integer(kind=__fvl_integer_kind__),parameter::maxnumtests=6
character(len=__fvl_character_len__)::fieldlabel,tabledirectory,intformat,realformat,doflabel,&
      testdirectories(maxnumtests),refdirectories(maxnumtests),&
      testmeshfilepath,testmeshfileform,testsolutionfiledirectory,testsolutionfileform,&
      testreportfiledirectory,testreportfileform,testoutputfiledirectory,testoutputfileform,&
      refmeshfilepath,refmeshfileform,refsolutionfiledirectory,refsolutionfileform,&
      refreportfiledirectory,refreportfileform
integer(kind=__fvl_integer_kind__)::i,j,nummeshes,convergencetype,reftype,fixconstant,fieldtype,tableprecision,unit,&
...
real(kind=__fvl_real_kind__)::aux,cellarea,meshareas(maxnumtests),&
      fixpointresid(maxnumtests),linearsystemresid(maxnumtests),&
      meshestotaltime(maxnumtests),modelstotaltime(maxnumtests),schemestotaltime(maxnumtests),&
      solutiontotaltime(maxnumtests),postproctotaltime(maxnumtests),executiontotaltime(maxnumtests),&
      refmeshestotaltime(maxnumtests),refmodelstotaltime(maxnumtests),refschemestotaltime(maxnumtests),&
      reftypetotaltime(maxnumtests),refpostproctotaltime(maxnumtests),refexecutiontotaltime(maxnumtests),&
      meshestotaltimeratio(maxnumtests),modelstotaltimeratio(maxnumtests),schemestotaltimeratio(maxnumtests),&
      solutiontotaltimeratio(maxnumtests),postproctotaltimeratio(maxnumtests),executiontotaltimeratio(maxnumtests),&
...
real(kind=__fvl_real_kind__),allocatable,dimension(:)::scalarrefsolution,scalartesterror
real(kind=__fvl_real_kind__),allocatable,dimension(:,:)::vectorrefsolution,vectortesterror
...
type(fvl_scalarmodel2d)::scalartestmodel,scalarrefmodel
type(fvl_vectormodel2d)::vectortestmodel,vectorrefmodel
type(fvl_scalarfield2d_selector)::scalarmodel_fieldselector
type(fvl_scalarboundcond2d_selector)::scalarmodel_boundcondselector
type(fvl_vectorfield2d_selector)::vectormodel_fieldselector
type(fvl_vectorboundcond2d_selector)::vectormodel_boundcondselector
...
reftype                       = controldictfile%getvalue("post","reference_type",1)
...
do i=1,maxnumtests
      testdirectories(i) = controldictfile%getvalue("post","test_directory_"//fvl_trim(fvl_char(i)),"test")
end do
do i=1,maxnumtests
      refdirectories(i) = controldictfile%getvalue("post","reference_directory_"//fvl_trim(fvl_char(i)),"reference")
end do

! read test case parameters
testmeshfilepath              = controldictfile%getvalue("test","mesh_file_path","mesh/mesh.fvm2.bin")
testmeshfileform              = controldictfile%getvalue("test","mesh_file_form","binary")
testsolutionfiledirectory     = controldictfile%getvalue("test","solution_file_directory","output/")
! testsolutionfiledirectory     = fvl_trim(solutionfiledirectory)//fvl_trim(fvl_shortchar(time))
testsolutionfileform          = controldictfile%getvalue("test","solution_file_form","binary")
testreportfiledirectory       = controldictfile%getvalue("test","report_file_directory","output/")
! testreportfiledirectory       = fvl_trim(testreportfiledirectory)//fvl_trim(fvl_shortchar(time))
testreportfileform            = controldictfile%getvalue("test","report_file_form","ascii")
testoutputfiledirectory       = controldictfile%getvalue("test","output_file_directory","output/")
! testoutputfiledirectory       = fvl_trim(testoutputfiledirectory)//fvl_trim(fvl_shortchar(time))
testoutputfileform            = controldictfile%getvalue("test","output_file_form","binary")

! read reference case parameters
refmeshfilepath               = controldictfile%getvalue("reference","mesh_file_path","mesh/mesh.fvm2.bin")
refmeshfileform               = controldictfile%getvalue("reference","mesh_file_form","binary")
refsolutionfiledirectory      = controldictfile%getvalue("reference","solution_file_directory","output/")
! refsolutionfiledirectory      = fvl_trim(refsolutionfiledirectory)//fvl_trim(fvl_shortchar(time))
refsolutionfileform           = controldictfile%getvalue("reference","solution_file_form","binary")
refreportfiledirectory        = controldictfile%getvalue("reference","report_file_directory","output/")
! refreportfiledirectory        = fvl_trim(refreportfiledirectory)//fvl_trim(fvl_shortchar(time))
refreportfileform             = controldictfile%getvalue("reference","report_file_form","ascii")
...
do i=1,maxnumtests
      ...
      if(exist) then
            ...
            ! check mesh path
            if(i>1) then
                  if(fvl_trim(testdirectories(i))==fvl_trim(testdirectories(i-1))) then
                        go to 1000
                  end if
            end if
            ...
            ! read mesh
            mesh = fvl_mesh2d(filepath=fvl_trim(testdirectories(i))//"/"//fvl_trim(testmeshfilepath),fileform=testmeshfileform,&
                  dictfile=meshesdictfile,dictlabel="test_mesh_"//fvl_trim(fvl_char(i)))
      ...
      if(reftype==2) then
            ...
            if(i>1) then
                  if(fvl_trim(refdirectories(i))==fvl_trim(refdirectories(i-1))) then
                        go to 2000
                  end if
            end if
            ...
            ! read mesh
            mesh = fvl_mesh2d(filepath=fvl_trim(refdirectories(i))//"/"//fvl_trim(refmeshfilepath),fileform=refmeshfileform,&
                  dictfile=meshesdictfile,dictlabel="reference_mesh_"//fvl_trim(fvl_char(i)))
            ...
            ! ! mesh locator
            ! meshlocator = fvl_meshlocator2d(mesh=refmesh,dictfile=postdictfile,dictlabel="mesh_locator")
            ...
            !
            ! reconstruction map
            ! reconstmap = fvl_reconstmap2d(patch=domain_patch,mesh=refmesh,meshlocator=meshlocator,&
            !       dictfile=postdictfile,dictlabel"reconst_map")
...
#include "models.f"
      ...
      if(fieldtype==1) then
            ...
            scalarrefmodel    = fvl_scalarmodel2d(mesh=mesh,fieldtype=fvl_field2d_cellmeansfield,&
                                    modeldictfile=modelsdictfile,modeldictlabel="reference_model",&
                                    patchesdictfile=meshesdictfile,patchesdictlabel="patches",&
                                    fieldselector=scalarmodel_fieldselector,boundcondselector=scalarmodel_boundcondselector,&
                                    label=fieldlabel,filepath=fvl_trim(refdirectories(i))//"/"//fvl_trim(refsolutionfiledirectory),&
                                    fileform=refsolutionfileform)

      else

            ! reference vector model
            vectorrefmodel   = fvl_vectormodel2d(mesh=mesh,fieldtype=fvl_field2d_cellmeansfield,&
                                    modeldictfile=modelsdictfile,modeldictlabel="reference_model",&
                                    patchesdictfile=meshesdictfile,patchesdictlabel="patches",&
                                    fieldselector=vectormodel_fieldselector,boundcondselector=vectormodel_boundcondselector,&
                                    label=fieldlabel,filepath=fvl_trim(refdirectories(i))//"/"//fvl_trim(refsolutionfiledirectory),&
                                    fileform=refsolutionfileform)
      ...
      if(reftype==2) then
            ...
            if(fieldtype==1) then
                  ...
                  ! call reconstmap % mapcellmeanvalues(mesh,scalarrefsolution)

            else

                  ! ! map reference solution
                  ! call reconstmap % mapcellmeanvalues(mesh,vectorrefsolution)
      ...
      if(fieldtype==1) then
            ...
            ! test scalar model
            scalartestmodel   = fvl_scalarmodel2d(mesh=mesh,fieldtype=fvl_field2d_cellmeansfield,&
                                    modeldictfile=modelsdictfile,modeldictlabel="test_model",&
                                    patchesdictfile=meshesdictfile,patchesdictlabel="patches",&
                                    fieldselector=scalarmodel_fieldselector,boundcondselector=scalarmodel_boundcondselector,&
                                    label=fvl_trim(fieldlabel)//"_error",filepath=fvl_trim(testdirectories(i))//"/"//fvl_trim(testsolutionfiledirectory),&
                                    fileform=testsolutionfileform,calculatedfields=.true.)

      else

            ! test vector model
            vectortestmodel   = fvl_vectormodel2d(mesh=mesh,fieldtype=fvl_field2d_cellmeansfield,&
                                    modeldictfile=modelsdictfile,modeldictlabel="test_model",&
                                    patchesdictfile=meshesdictfile,patchesdictlabel="patches",&
                                    fieldselector=vectormodel_fieldselector,boundcondselector=vectormodel_boundcondselector,&
                                    label=fvl_trim(fieldlabel)//"_error",filepath=fvl_trim(testdirectories(i))//"/"//fvl_trim(testsolutionfiledirectory),&
      ...
      if(fieldtype==1) then
            ...
            if(fixconstant==1) then
                  call scalartestmodel%evalcellmeanvalues(scalartesterror)
                  call scalartestmodel%subvalue(fvl_omp_dot(scalartesterror,mesh%getcellareas())/meshareas(i))
                  call scalarrefmodel%evalcellmeanvalues(scalartesterror)
                  call scalarrefmodel%subvalue(fvl_omp_dot(scalartesterror,mesh%getcellareas())/meshareas(i))
            end if

            ! compute errors
            call scalartestmodel%submodel(scalarrefmodel)
            call scalartestmodel%evalcellmeanvalues(scalartesterror)
      ...
      ! ----------------------------------------------------------------------------
            ...
            ! fix constant
            if(fixconstant==1) then
                  call vectortestmodel%evalcellmeanvalues(vectortesterror)
                  call vectortestmodel%subvalue(fvl_omp_dot(vectortesterror,mesh%getcellareas())/meshareas(i))
                  call vectorrefmodel%evalcellmeanvalues(vectortesterror)
                  call vectorrefmodel%subvalue(fvl_omp_dot(vectortesterror,mesh%getcellareas())/meshareas(i))
            end if

            ! compute errors
            call vectortestmodel%submodel(vectorrefmodel)
            call vectortestmodel%evalcellmeanvalues(vectortesterror)
            ...
            do j=1,numcells(i)
                  cellarea = mesh%getcellarea(j)
                  errorx1(i) = errorx1(i)+abs(vectortesterror(1,j))*cellarea
                  errorx2(i) = errorx2(i)+(vectortesterror(1,j)**2.0)*cellarea
                  errorxinf(i) = max(errorxinf(i),abs(vectortesterror(1,j)))
                  errory1(i) = errory1(i)+abs(vectortesterror(2,j))*cellarea
                  errory2(i) = errory2(i)+(vectortesterror(2,j)**2.0)*cellarea
                  erroryinf(i) = max(erroryinf(i),abs(vectortesterror(2,j)))
                  aux=fvl_vectormag2(vectortesterror(:,j))
                  errormag1(i) = errormag1(i)+aux*cellarea
                  errormag2(i) = errormag2(i)+(aux**2.0)*cellarea
                  errormaginf(i) = max(errormaginf(i),aux)
            end do
            errorx1(i) = errorx1(i)/meshareas(i)
            errorx2(i) = sqrt(errorx2(i)/meshareas(i))
            errory1(i) = errory1(i)/meshareas(i)
            errory2(i) = sqrt(errory2(i)/meshareas(i))
            errormag1(i) = errormag1(i)/meshareas(i)
            errormag2(i) = sqrt(errormag2(i)/meshareas(i))
      ...
      ! inspect file
      inquire(file=fvl_trim(refdirectories(i))//"/"//fvl_trim(refreportfiledirectory)//"/time.fvd",exist=exist)
      ...
      if(exist) then
            ...
            ! read file
            paramfile = fvl_keys_file(path=fvl_trim(refdirectories(i))//"/"//fvl_trim(refreportfiledirectory)//"/time.fvd",&
                  form=refreportfileform,action="read")
            call paramfile % open()
            ...
            reftypetotaltime(i) = paramfile%getvalue("solution_total",0.0d0)
      ...
      ! inspect file
      inquire(file=fvl_trim(testdirectories(i))//"/"//fvl_trim(testreportfiledirectory)//"/time.fvd",exist=exist)
      ...
      if(exist) then
            ...
            ! read file
            paramfile = fvl_keys_file(path=fvl_trim(testdirectories(i))//"/"//fvl_trim(testreportfiledirectory)//"/time.fvd",&
                  form=testreportfileform,action="read")
            call paramfile % open()
            call paramfile % readkeys()
            ...
            if(reftypetotaltime(i)>0.0d0) then
                  solutiontotaltimeratio(i) = solutiontotaltime(i)/reftypetotaltime(i)
            else
                  solutiontotaltimeratio(i) = -1.0d0
            end if
      ...
      ! inspect file
      inquire(file=fvl_trim(refdirectories(i))//"/"//fvl_trim(refreportfiledirectory)//"/memory.fvd",exist=exist)
      ...
      if(exist) then
            ...
            ! read file
            paramfile = fvl_keys_file(path=fvl_trim(refdirectories(i))//"/"//fvl_trim(refreportfiledirectory)//"/memory.fvd",&
                  form=refreportfileform,action="read")
            call paramfile % open()
            call paramfile % readkeys()
      ...
      ! inspect file
      inquire(file=fvl_trim(testdirectories(i))//"/"//fvl_trim(testreportfiledirectory)//"/memory.fvd",exist=exist)
      ...
      if(exist) then
            ...
            ! read file
            paramfile = fvl_keys_file(path=fvl_trim(testdirectories(i))//"/"//fvl_trim(testreportfiledirectory)//"/memory.fvd",&
                  form=testreportfileform,action="read")
            call paramfile % open()
            call paramfile % readkeys()
      ...
      ! inspect file
      inquire(file=fvl_trim(testdirectories(i))//"/"//fvl_trim(testreportfiledirectory)//"/linear_solver.fvd",exist=exist)
      ...
      if(exist) then
            ...
            ! read file
            file = fvl_file(path=fvl_trim(testdirectories(i))//"/"//fvl_trim(testreportfiledirectory)//"/linear_solver.fvd",&
                  form=testreportfileform,action="read")
            call file % open()
      ...
      ! inspect file
      inquire(file=fvl_trim(testdirectories(i))//"/"//fvl_trim(testreportfiledirectory)//"/"//fvl_trim(fieldlabel)//"fixpointiterator.fvd",exist=exist)
      ...
      if(exist) then
            ...
            ! read file
            file = fvl_file(path=fvl_trim(testdirectories(i))//"/"//fvl_trim(testreportfiledirectory)//"/"//fvl_trim(fieldlabel)//"fixpointiterator.fvd",&
                  form=testreportfileform,action="read")
            call file % open()
